package coreNLP.ParserExamples;
...
class ParserDemo2 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    String[] options = { "-maxLength", "80", "-retainTmpSubcategories" , "-outputFormat", "wordsAndTags,penn,typedDependencies"};
    LexicalizedParser lp = LexicalizedParser.loadModel(grammar, options);
    TreebankLanguagePack tlp = lp.getOp().langpack();
    GrammaticalStructureFactory gsf = tlp.grammaticalStructureFactory();
    
    String s1 = "The strongest rain ever recorded in India shut down the financial hub of Mumbai, snapped communication lines, closed airports and forced thousands of people to sleep in their offices or walk home during the night, officials said today.";
    System.out.println(s1.split("\\s+").length);
    Tree parsex = lp.parse(s1);
    parsex.pennPrint();
    System.out.println();
    System.out.println();
    GrammaticalStructure grammaticalStructure = gsf.newGrammaticalStructure(parsex);
    List<TypedDependency> typedDependencies = grammaticalStructure.typedDependenciesCCprocessed();
    printList(typedDependencies);
    System.out.println();
    System.out.println();
    printList(parsex.taggedYield());
    System.out.println();
    System.out.println();
    System.out.println();
